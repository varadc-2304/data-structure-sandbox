...
import CSCANVisualization from "./cscan/CSCANVisualization";
import CSCANAlgorithmInfo from "./cscan/CSCANAlgorithmInfo";
import { useCSCANVisualizer } from "./cscan/useCSCANVisualizer";
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="page-container pt-20">
        <div className="mb-6 animate-slide-in">
          ...
          <h1 className="text-3xl font-bold text-foreground mb-2">C-SCAN (Circular SCAN)</h1>
          <p className="text-muted-foreground text-sm">Visualize the C-SCAN disk scheduling algorithm. Like SCAN but returns to the beginning after reaching the end, providing more uniform wait times.</p>
        </div>
        ...
        <Tabs defaultValue="visualizer" className="w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {cscanOrder.length > 0 && (
                      <div className="space-y-2">
                        ...
                        <Slider value={[currentStep + 1]} onValueChange={([value]) => goToStep(value - 1)} max={cscanOrder.length} min={0} step={1} className="w-full" />
                      </div>
                    )}
                  </CardContent>
                </Card>
              ...
              <div className="md:col-span-2">
                <div className="bg-card rounded-2xl shadow-md p-4 border border-border animate-scale-in" style={{ animationDelay: "0.2s" }}>
                  <div className="flex flex-wrap gap-3 mb-4">
                    ...
                    <Button onClick={resumeSimulation} variant="outline" disabled={requestQueue.length === 0 || isPlaying || currentStep >= cscanOrder.length - 1} size="sm">
                      <Play className="mr-2 h-3 w-3" />
                      Resume
                    </Button>
                    ...
                    <Button onClick={nextStep} variant="outline" disabled={requestQueue.length === 0 || currentStep >= cscanOrder.length - 1} size="sm">
                    ...
                    <div className="ml-auto flex items-center bg-secondary px-2 py-1 rounded-md">
                      ...
                      <span className="text-foreground font-medium text-sm">
                        Step: {currentStep + 1} / {cscanOrder.length || 1}
                      </span>
                    </div>
                  </div>
                              ...
                              <tr key={index} className={index <= currentStep ? "bg-primary/10" : "bg-card"}>
                                ...
                                <td className="px-3 py-2 whitespace-nowrap">
                                  {index > 0 && index <= currentStep
                                    ? Math.abs(request.position - cscanOrder[index - 1].position)
              ...
              <div className="md:col-span-3">
                <div className="bg-card rounded-2xl shadow-md p-4 border border-border animate-scale-in text-sm" style={{ animationDelay: "0.4s" }}>
                  <h2 className="text-lg font-semibold mb-2 text-foreground">About C-SCAN Algorithm</h2>
                  <p className="text-muted-foreground mb-3 text-sm">C-SCAN (Circular SCAN) is like SCAN but returns to the beginning after reaching the end, providing more uniform wait times than SCAN.</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    ...
                    <Card className="bg-secondary">
                      ...
                      <CardContent className="py-2 px-3">
                        <ul className="list-disc pl-4 text-muted-foreground space-y-1">
                          <li>May have longer seek times</li>
                          <li>Returns to beginning after end</li>
          ...
          <TabsContent value="algorithm" className="mt-0">
                <CSCANAlgorithmInfo type="algorithm" />
              </TabsContent>
            </Tabs>
      </div>
    </div>
