        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-5 gap-1">
              ...
              <Button size="sm" onClick={onTogglePlayPause} disabled={clookOrderLength === 0} className="bg-drona-green hover:bg-drona-green/90 flex items-center justify-center">
                {isPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
              </Button>
              <Button variant="outline" size="sm" onClick={onFastForward} className="flex items-center justify-center">
                <FastForward className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="sm" onClick={onNextStep} disabled={currentStep >= clookOrderLength - 1} className="flex items-center justify-center">
          ...
          {clookOrderLength > 0 && (
            <div className="space-y-2">
              <Label>
                Step: {currentStep + 1} of {clookOrderLength}
              </Label>
              <Slider value={[currentStep + 1]} onValueChange={([value]) => onGoToStep(value - 1)} max={clookOrderLength} min={0} step={1} className="w-full" />
            </div>
          )}
        </div>
      </CardContent>
