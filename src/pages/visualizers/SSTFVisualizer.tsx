    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="page-container pt-20">
        <div className="mb-6 animate-slide-in">
          <Link to="/disk-scheduling" className="flex items-center text-arena-green hover:underline mb-2">
          ...
          <p className="text-arena-gray text-sm">
            Visualize the Shortest Seek Time First scheduling algorithm. The disk request closest to the current head position is served first.
          </p>
        </div>
        ...
        <Tabs defaultValue="visualizer" className="w-full">
          ...
          <TabsContent value="visualizer" className="mt-0">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Input Section - Takes 1/3 of the screen */}
              <div className="md:col-span-1">
                <Card className="shadow-md">
                  <CardHeader>
                    <CardTitle className="flex items-center">
                      <Settings className="h-5 w-5 mr-2 text-arena-green" />
                      Configuration
                    </CardTitle>
                    <CardDescription>Set up the disk scheduling parameters</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex items-center mt-1 space-x-2">
                          <Slider 
                            ...
                            className="flex-grow"
                          />
                          <span className="text-sm font-medium bg-arena-light px-2 py-1 rounded w-12 text-center">
                            {diskSize}
                          </span>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between mb-2">
                          ...
                          <span className="text-sm text-arena-gray">{speed}x</span>
                        </div>
                        <Slider 
                          min={0.5} 
                          max={5} 
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 gap-4">
                      <div className="bg-arena-light p-4 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-arena-dark">{totalSeekTime} cylinders</p>
                      </div>
                      <div className="bg-arena-light p-4 rounded-lg">
                        <p className="text-sm text-arena-gray">Average Seek Time</p>
                        <p className="text-2xl font-bold text-arena-dark">
              ...
              {/* Visualization Area - Takes 2/3 of the screen */}
              <div className="md:col-span-2">
                <Card className="h-full shadow-md">
                  <CardHeader className="border-b">
                    <CardTitle className="flex items-center">
                      <HardDriveDownload className="h-5 w-5 mr-2 text-arena-green" />
                  ...
                  <CardContent className="pt-6">
                    <div className="mb-6">
                      <h3 className="text-sm font-medium text-arena-gray mb-2">Disk Visualization</h3>
                      <div className="relative h-20 bg-gradient-to-r from-arena-light to-blue-50 rounded-lg overflow-hidden mb-4 border border-gray-200">
                        ...
                        <div 
                          className="absolute top-0 h-full w-1.5 bg-arena-green transition-all duration-500 z-30"
                          style={{ left: `${(currentHeadPosition / diskSize) * 100}%` }}
                        >
                          <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 text-xs font-medium text-arena-green whitespace-nowrap">
                            Current Head: {currentHeadPosition}
                          </div>
                          {/* Top part of the disk head */}
                          <div className="absolute -top-2.5 left-1/2 transform -translate-x-1/2 w-5 h-5 bg-arena-green rounded-full border-2 border-white shadow-md"></div>
                          {/* Bottom part of the disk head */}
                          <div className="absolute -bottom-2.5 left-1/2 transform -translate-x-1/2 w-5 h-5 bg-arena-green rounded-full border-2 border-white shadow-md"></div>
                        </div>
                        ...
                        {requestQueue.map((req, idx) => (
                          <div 
                            ...
                            className={cn(
                              ...
                              req.processed ? "bg-arena-green/70" : "bg-gray-400/70",
                              req.current && "ring-2 ring-arena-green ring-offset-1 scale-125"
                            )}
                            style={{ 
                      ...
                      <div className="flex justify-between text-xs text-arena-gray">
                        <span>0</span>
                    ...
                    <div className="mb-6">
                      <h3 className="text-sm font-medium text-arena-gray mb-2">Request Queue</h3>
                      <div className="flex flex-wrap gap-2 mb-4">
                        {requestQueue.map((request, idx) => (
                          <Badge 
                            ...
                            className={cn(
                              request.current && "bg-arena-green animate-pulse",
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-arena-gray mb-2">Seek Operations</h3>
                      <div className="max-h-[200px] overflow-y-auto border border-gray-200 rounded-lg shadow-inner">
                        {seekHistory.length === 0 ? (
                          <div className="p-4 text-center text-gray-400">No operations yet</div>
                        ) : (
                          <table className="w-full">
                            <thead className="bg-arena-light sticky top-0 z-10">
                              <tr>
                              ...
                              {seekHistory.map((seek, idx) => (
                                <tr 
                                  ...
                                  className={cn(
                                    ...
                                    idx === seekHistory.length - 1 && 'bg-arena-green/10'
                                  )}
                                >
                                  <td className="px-3 py-2 text-sm">{idx + 1}</td>
                                  <td className="px-3 py-2 text-sm">{seek.from}</td>
                                  <td className="px-3 py-2 text-sm">{seek.to}</td>
                                  <td className="px-3 py-2 text-sm">{seek.distance} cylinders</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
          ...
          <TabsContent value="algorithm">
            <Card className="shadow-md">
              ...
              <CardContent>
                <div className="mb-6">
                  <h3 className="font-medium text-arena-dark mb-2">How it works</h3>
                ...
                <div className="mb-6">
                  <h3 className="font-medium text-arena-dark mb-2">Pseudocode</h3>
...
{`function SSTF_DiskScheduling(requestQueue, initialHeadPosition):
    ...
    return totalSeekTime, processedRequests`}
                ...
                <div>
                  <h3 className="font-medium text-arena-dark mb-2">Advantages and Disadvantages</h3>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Card className="bg-arena-light">
                      <CardHeader className="py-2 px-3">
                        <CardTitle className="text-xs font-medium">Advantages</CardTitle>
                      </CardHeader>
                      <CardContent className="py-2 px-3">
                        <ul className="list-disc pl-4 text-arena-gray space-y-1 text-xs">
                    ...
                    <Card className="bg-arena-light">
                      <CardHeader className="py-2 px-3">
                        <CardTitle className="text-xs font-medium">Disadvantages</CardTitle>
                      </CardHeader>
                      <CardContent className="py-2 px-3">
                        <ul className="list-disc pl-4 text-arena-gray space-y-1 text-xs">
