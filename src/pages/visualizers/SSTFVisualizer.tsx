...
import SSTFVisualization from "./sstf/SSTFVisualization";
import SSTFAlgorithmInfo from "./sstf/SSTFAlgorithmInfo";
import { useSSTFVisualizer } from "./sstf/useSSTFVisualizer";
      ...
      <div className="page-container pt-20">
        ...
        <Tabs defaultValue="visualizer" className="w-full">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {sstfOrder.length > 0 && (
                      <div className="space-y-2">
                        ...
                        <Slider value={[currentStep + 1]} onValueChange={([value]) => goToStep(value - 1)} max={sstfOrder.length} min={0} step={1} className="w-full" />
                      </div>
                    )}
                  </CardContent>
                </Card>
              ...
              <div className="md:col-span-2">
                <div className="bg-card rounded-2xl shadow-md p-4 animate-scale-in border border-border" style={{ animationDelay: "0.2s" }}>
                  <div className="flex flex-wrap gap-3 mb-4">
                    ...
                    <Button onClick={resumeSimulation} variant="outline" disabled={requestQueue.length === 0 || isPlaying || currentStep >= sstfOrder.length - 1} size="sm">
                      <Play className="mr-2 h-3 w-3" />
                      Resume
                    </Button>
                              ...
                              <tr key={index} className={index <= currentStep ? "bg-primary/10" : "bg-card"}>
                                ...
                                <td className="px-3 py-2 whitespace-nowrap">
                                  {index > 0 && index <= currentStep
                                    ? Math.abs(request.position - sstfOrder[index - 1].position)
          ...
          <TabsContent value="algorithm" className="mt-0">
                <SSTFAlgorithmInfo type="algorithm" />
              </TabsContent>
            </Tabs>
      </div>
    </div>
