...
import { useDecisionTreeVisualizer, TreeNode } from "./decision-tree/useDecisionTreeVisualizer";

const DecisionTreeVisualizer = () => {
  const {
    state: { tree, isPlaying, currentStep, speed, maxSteps },
    actions: { setSpeed, togglePlayPause, nextStep, prevStep, goToStep, skipToStart, skipToEnd, resetVisualization, getStepDescription },
  } = useDecisionTreeVisualizer();
  ...
  const renderNode = (node: TreeNode | undefined, x: number, y: number): React.ReactNode => {
    ...
    const nodeColor = node.isLeaf ? (node.prediction === "Yes" ? "bg-green-700" : "bg-red-500") : "bg-blue-500";

    return (
      <g key={node.id}>
        <circle cx={x} cy={y} r="30" className={`${nodeColor} ${node.active ? "opacity-100" : "opacity-30"} transition-all duration-500`} fill="currentColor" />
        <text x={x} y={y - 5} textAnchor="middle" className={`text-xs font-bold fill-white ${node.active ? "opacity-100" : "opacity-30"}`}>
          {node.isLeaf ? node.prediction : node.feature}
        </text>
        <text x={x} y={y + 8} textAnchor="middle" className={`text-xs fill-white ${node.active ? "opacity-100" : "opacity-30"}`}>
          {node.isLeaf ? "" : `≤ ${node.threshold}`}
        </text>
        <text x={x} y={y + 50} textAnchor="middle" className={`text-xs font-medium ${node.active ? "text-drona-dark" : "text-gray-400"}`}>
          {node.samples} samples
        </text>
        {node.left && (
          <>
            <line x1={x} y1={y + 30} x2={x - 100} y2={y + 100} className={`stroke-2 ${node.active && node.left.active ? "stroke-drona-green" : "stroke-gray-300"} transition-colors duration-500`} />
            <text x={x - 50} y={y + 70} textAnchor="middle" className={`text-xs font-medium ${node.active ? "text-drona-green" : "text-gray-400"}`}>
              Yes
            </text>
        ...
        {node.right && (
          <>
            <line x1={x} y1={y + 30} x2={x + 100} y2={y + 100} className={`stroke-2 ${node.active && node.right.active ? "stroke-drona-green" : "stroke-gray-300"} transition-colors duration-500`} />
            <text x={x + 50} y={y + 70} textAnchor="middle" className={`text-xs font-medium ${node.active ? "text-drona-green" : "text-gray-400"}`}>
              No
            </text>
    ...
    <div className="min-h-screen bg-gradient-to-br from-drona-light via-white to-drona-light">
      ...
      <div className="page-container mt-20">
        <div className="mb-8">
          ...
          <p className="text-lg text-drona-gray">Visualize how decision trees make predictions through a series of if-else conditions</p>
        </div>
            ...
            <Card className="shadow-lg border-2 border-drona-green/20">
              ...
              <CardContent className="space-y-4 pt-6">
                ...
                <div className="space-y-2">
                  ...
                  <Slider value={[currentStep + 1]} onValueChange={([value]) => goToStep(value - 1)} max={maxSteps} min={0} step={1} className="w-full" />
                </div>
                <div className="space-y-2">
